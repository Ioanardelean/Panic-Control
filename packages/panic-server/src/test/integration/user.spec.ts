...
describe('GET /profile', () => {
  it('respond with 200 authorized ', async () => {
    await request(app.callback()).get('/users/profile').set('Authorization', `Bearer ${token}`)
      .expect((res) => {
...
describe('DELETE /:id  ', () => {
  it('respond with 404 bad request user not exist in db ', async () => {
    await request(app.callback()).del(`/users/${14525}`).set('Authorization', `Bearer ${token}`)
      .expect(400);
  });
});
describe('PUT /:id', () => {
  it('respond with 200 updated ', async () => {
    await request(app.callback()).put(`/users/${id}`).set('Authorization', `Bearer ${token}`)
...
describe('DELETE /:id', () => {
  it('respond with 401 unauthorized ', async () => {
    await request(app.callback()).del(`/users/${id}`).set('Authorization', `Bearer ${token}`)
      .expect(401);
  });
});
